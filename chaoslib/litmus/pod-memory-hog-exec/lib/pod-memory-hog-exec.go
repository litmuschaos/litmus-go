...
func injectChaosInSerialMode(ctx context.Context, experimentsDetails *experimentTypes.ExperimentDetails, targetPodList corev1.PodList, clients clients.ClientSets, resultDetails *types.ResultDetails, eventsDetails *types.EventDetails, chaosDetails *types.ChaosDetails) error {
	...
	default:
		...
		loop:
			for {
				...
				case err := <-stressErr:
					...
					if err != nil {
						...
						span.SetStatus(codes.Error, "Chaos injection failed")
						err := cerrors.Error{ErrorCode: cerrors.ErrorTypeChaosInject, Target: fmt.Sprintf("podName: %s, namespace: %s, container: %s", pod.Name, pod.Namespace, experimentsDetails.TargetContainer), Reason: fmt.Sprintf("failed to stress memory of target pod: %s", err.Error())}
						return err
					}
				case <-signChan:
			...
			if err := killStressMemorySerial(experimentsDetails.TargetContainer, pod.Name, pod.Namespace, experimentsDetails.ChaosKillCmd, clients, chaosDetails); err != nil {
				span.SetStatus(codes.Error, "could not revert memory stress")
				span.RecordError(err)
	...
	for {
		...
		case err := <-stressErr:
			...
			if err != nil {
				...
				span.SetStatus(codes.Error, "failed to stress memory of target pod")
				err := cerrors.Error{ErrorCode: cerrors.ErrorTypeChaosInject, Reason: fmt.Sprintf("failed to stress memory of target pod: %s", err.Error())}
				span.RecordError(err)
				return err
			}
		case <-signChan:
