...
func injectChaosInSerialMode(experimentsDetails *experimentTypes.ExperimentDetails, instanceNameList []string, clients clients.ClientSets, resultDetails *types.ResultDetails, eventsDetails *types.EventDetails, chaosDetails *types.ChaosDetails) error {
	...
	default:
		...
		for duration < experimentsDetails.ChaosDuration {
			...
			log.Infof("[Info]: Target instanceID list, %v", instanceNameList)
			...
			for i, vmName := range instanceNameList {
				...
				log.Info("[Chaos]: Stopping the desired Azure instance")
				...
				log.Infof("[Wait]: Waiting for Azure instance '%v' to get in stopped state", vmName)
				...
				log.Infof("[Wait]: Waiting for Azure instance '%v' to get in running state", vmName)
				if err := azureStatus.WaitForAzureComputeUp(experimentsDetails.Timeout, experimentsDetails.Delay, experimentsDetails.SubscriptionID, experimentsDetails.ResourceGroup, vmName); err != nil {
					return errors.Errorf("unable to start the Azure instance, err: %v", err)
				}
			}
			duration = int(time.Since(ChaosStartTimeStamp).Seconds())
	...
	default:
		...
		for duration < experimentsDetails.ChaosDuration {
			...
			for _, vmName := range instanceNameList {
				// Stopping the Azure instance
				log.Infof("[Chaos]: Stopping the desired Azure instance")
				if err := azureStatus.AzureInstanceStop(experimentsDetails.Timeout, experimentsDetails.Delay, experimentsDetails.SubscriptionID, experimentsDetails.ResourceGroup, vmName); err != nil {
					return errors.Errorf("unable to stop azure instance, err: %v", err)
				}
			}
...
// 	err = azureStatus.AzureInstanceStop(experimentsDetails.Timeout, experimentsDetails.Delay, experimentsDetails.SubscriptionID, experimentsDetails.ResourceGroup, vmName)
// 	if err != nil {
...
// 	err = azureStatus.AzureInstanceStart(experimentsDetails.Timeout, experimentsDetails.Delay, experimentsDetails.SubscriptionID, experimentsDetails.ResourceGroup, vmName)
// 	if err != nil {
