...
func InjectChaos(experimentsDetails *experimentTypes.ExperimentDetails, diskNameList []string, diskList *[]compute.DataDisk, clients clients.ClientSets, resultDetails *types.ResultDetails, eventsDetails *types.EventDetails, chaosDetails *types.ChaosDetails) error {
	...
	for duration < experimentsDetails.ChaosDuration {
		...
		log.Info("[Chaos]: Detaching the Virtual disk back from the instance")
		if err = azureStatus.DetachDisk(experimentsDetails.SubscriptionID, experimentsDetails.ResourceGroup, experimentsDetails.AzureInstanceName, diskNameList); err != nil {
		...
		// diskState, err := ebs.GetEBSStatus(volumeID, ec2InstanceID, experimentsDetails.Region)
		// if err != nil {
