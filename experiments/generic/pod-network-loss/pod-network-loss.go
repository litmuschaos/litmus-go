...
func main() {
	...
	if err != nil {
		...
		resultDetails.FailStep = "Updating the chaos result of pod-network-loss experiment (SOT)"
	...
	if experimentsDetails.ChaosLib == "pumba" {
		...
		if err != nil {
			...
			resultDetails.FailStep = "Including the pumba lib for pod-network-loss"
			result.ChaosResult(&chaosDetails, clients, &resultDetails, "EOT")
			return
		}
