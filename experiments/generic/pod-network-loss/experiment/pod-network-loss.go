...
func PodNetworkLoss(clients clients.ClientSets) {
	...
	if err := result.ChaosResult(&chaosDetails, clients, &resultDetails, "SOT"); err != nil {
		...
		result.RecordAfterFailure(&chaosDetails, &resultDetails, result.ResultUpdatePreChaos, clients, &eventsDetails)
		return
	}
	...
	if chaosDetails.DefaultHealthCheck {
		...
		if err := status.AUTStatusCheck(experimentsDetails.AppNS, experimentsDetails.AppLabel, experimentsDetails.TargetContainer, experimentsDetails.Timeout, experimentsDetails.Delay, clients, &chaosDetails); err != nil {
			...
			result.RecordAfterFailure(&chaosDetails, &resultDetails, result.AUTStatusCheckPreChaos, clients, &eventsDetails)
			return
		}
	}
	...
	if experimentsDetails.EngineName != "" {
		...
		if len(resultDetails.ProbeDetails) != 0 {
			...
			if err := probe.RunProbes(&chaosDetails, clients, &resultDetails, "PreChaos", &eventsDetails); err != nil {
				...
				events.GenerateEvents(&eventsDetails, clients, &chaosDetails, "ChaosEngine")
				result.RecordAfterFailure(&chaosDetails, &resultDetails, result.ProbeExecutionPreChaos, clients, &eventsDetails)
				return
			}
	...
	case experimentsDetails.ChaosLib == "pumba" && experimentsDetails.ContainerRuntime == "docker":
		if err := pumbaLIB.PodNetworkLossChaos(&experimentsDetails, clients, &resultDetails, &eventsDetails, &chaosDetails); err != nil {
			...
			failStep := "[chaos]: failed to execute pumba chaoslib"
			result.RecordAfterFailure(&chaosDetails, &resultDetails, failStep, clients, &eventsDetails)
			return
		}
	case experimentsDetails.ChaosLib == "litmus":
		if failStep, err := litmusLIB.PodNetworkLossChaos(&experimentsDetails, clients, &resultDetails, &eventsDetails, &chaosDetails); err != nil {
			log.Errorf("Chaos injection failed, err: %v", err)
			result.RecordAfterFailure(&chaosDetails, &resultDetails, failStep, clients, &eventsDetails)
			return
		}
	default:
		...
		failStep := fmt.Sprintf("[chaos]: unsupported lib and container runtime due, {lib: '%v', containerRuntime: '%v'}", experimentsDetails.ChaosLib, experimentsDetails.ContainerRuntime)
		result.RecordAfterFailure(&chaosDetails, &resultDetails, failStep, clients, &eventsDetails)
		return
	}
	...
	if chaosDetails.DefaultHealthCheck {
		...
		if err := status.AUTStatusCheck(experimentsDetails.AppNS, experimentsDetails.AppLabel, experimentsDetails.TargetContainer, experimentsDetails.Timeout, experimentsDetails.Delay, clients, &chaosDetails); err != nil {
			...
			events.GenerateEvents(&eventsDetails, clients, &chaosDetails, "ChaosEngine")
			result.RecordAfterFailure(&chaosDetails, &resultDetails, result.AUTStatusCheckPostChaos, clients, &eventsDetails)
			return
		}
	}
	...
	if experimentsDetails.EngineName != "" {
		...
		if len(resultDetails.ProbeDetails) != 0 {
			if err := probe.RunProbes(&chaosDetails, clients, &resultDetails, "PostChaos", &eventsDetails); err != nil {
				...
				events.GenerateEvents(&eventsDetails, clients, &chaosDetails, "ChaosEngine")
				result.RecordAfterFailure(&chaosDetails, &resultDetails, result.ProbeExecutionPostChaos, clients, &eventsDetails)
				return
			}
			msg = common.GetStatusMessage(chaosDetails.DefaultHealthCheck, "AUT: Running", "Successful")
		}
