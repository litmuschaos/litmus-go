...
func RunProbes(chaosDetails *types.ChaosDetails, clients clients.ClientSets, resultDetails *types.ResultDetails, phase string, eventsDetails *types.EventDetails, conn *websocket.Conn) error {
	...
	case "prechaos", "duringchaos":
		for _, probe := range probes {
			if err := execute(probe, chaosDetails, clients, resultDetails, phase, conn); err != nil {
				return err
			}
		}
	default:
...
func execute(probe v1alpha1.ProbeAttributes, chaosDetails *types.ChaosDetails, clients clients.ClientSets, resultDetails *types.ResultDetails, phase string, conn *websocket.Conn) error {
	...
	case "cmdprobe":
		...
		if err = prepareCmdProbe(probe, clients, chaosDetails, resultDetails, phase, conn); err != nil {
			return errors.Errorf("probes failed, err: %v", err)
		}
	case "httpprobe":
